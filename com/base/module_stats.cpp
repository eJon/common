...
//            (C) SINA Inc.
//
//           file: mscom/base/module_stats.cc
//           desc: 
    ...
    range(key_range), _S(1000), _M(60000), _H(3600000), _D(86400000), h(handler)
{
    ...
    do {
	__start = (int64_t *)calloc(1, alloc_size);
    } while (!__start);
...
int module_stat::incrkey(int key, int64_t val) {
    ...
    keys_s_now[idx] += val;
    keys_s_trigger_cnt[idx]++;
    if (!keys_s_min[idx] || val < keys_s_min[idx])
	keys_s_min[idx] = val;
    if (!keys_s_max[idx] || val > keys_s_max[idx])
	keys_s_max[idx] = val;

    
    keys_m_now[idx] += val;
    keys_m_trigger_cnt[idx]++;
    if (!keys_m_min[idx] || val < keys_m_min[idx])
	keys_m_min[idx] = val;
    if (!keys_m_max[idx] || val > keys_m_max[idx])
	keys_m_max[idx] = val;


    keys_h_now[idx] += val;
    keys_h_trigger_cnt[idx]++;
    if (!keys_h_min[idx] || val < keys_h_min[idx])
	keys_h_min[idx] = val;
    if (!keys_h_max[idx] || val > keys_h_max[idx])
	keys_h_max[idx] = val;

    keys_d_now[idx] += val;
    keys_d_trigger_cnt[idx]++;
    if (!keys_d_min[idx] || val < keys_d_min[idx])
	keys_d_min[idx] = val;
    if (!keys_d_max[idx] || val > keys_d_max[idx])
	keys_d_max[idx] = val;
...
#define __get_module_key(t, __key, __op)				\
	...
	case AVG:							\
		...
		return keys_##t##_now[__idx] / keys_##t##_trigger_cnt[__idx]; \
	    }								\
...
#define __trigger_one_key_stat(t, interval, __key, __now_time)		\
	...
	if (__now_time - __timestamp > interval) {			\
		...
		getkey_##t(__key, NOW) > keys_##t##_threshold[idx])	\
		h->t##_warn(this, __key, keys_##t##_threshold[idx],	\
			    getkey_##t(__key, NOW),			\
			    getkey_##t(__key, AVG),			\
			    getkey_##t(__key, MIN),			\
			    getkey_##t(__key, MAX));			\
