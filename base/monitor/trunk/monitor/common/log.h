...
static const std::string loggerName = "monitor";
...
#define MONITOR_LOG_TRACE(trace_flag,format,...)  \
    if (trace_flag == TRACE_FLAG_ON) {  \
      MONITOR_LOG_NOTICE(format, ##__VA_ARGS__);  \
    } \
    else {  \
      MONITOR_LOG_DEBUG(format, ##__VA_ARGS__); \
    }

#define MONITOR_LOG_TRACEON(format,...)  \
    MONITOR_LOG_TRACE(TRACE_FLAG_ON, format, ##__VA_ARGS__)    
