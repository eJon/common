...
void classic_usage() {
  ...
  memcached_behavior_set(mem_instance, MEMCACHED_BEHAVIOR_REMOVE_FAILED_SERVERS, true); 
  // default is 5 
  ...
  memcached_behavior_set(mem_instance, MEMCACHED_BEHAVIOR_DEAD_TIMEOUT, 10); 
...
#define SET_OPERATION
    ...
    if (0 != ret) {
      print_msg("error in memcached_get", ret);
    } else {
      print_msg("success in memcached_get", ret);
    }
