...
{
    if (test) throw JsonParserError(s);
}

inline static void trueThenThrow(bool test, const std::string& s)
{
    if (test) throw JsonParserError(s.c_str());
}
...
void ParseJson(const std::string& is, size_t& pos, Any& ret)
{
    ...
    SHARELIB_JSON_THROW(JsonNothingError, "");
}
...
void ParseJson(const std::string& is, Any& ret)
{
    ...
    if (pos < is.size())
    {
        SHARELIB_JSON_THROW(JsonParserError, "Expect EOF but see more chars");
    }
}

void ToString(const Any& a, std::string &os, bool isCompact, const std::string& prefix)
{
    typeof(a.GetType()) &type = a.GetType();
    ...
    {
        os.append(prefix);
        os.append(ToString(AnyCast<bool>(a)));
    }
...
#define WRITE_DATA(tp)                                          \
    ...
    {                                                           \
        os.append(prefix);                                      \
        os.append(ToString(AnyCast<tp>(a)));    \
    }
    WRITE_DATA(double)
        WRITE_DATA(float)
        WRITE_DATA(uint16_t)
...
#define WRITE_DATA_AS(tp, as)                                           \
        ...
        {                                                               \
            os.append(prefix);                                          \
            os.append(ToString(static_cast<as>(AnyCast<tp>(a)))); \
        }
...
void ParseJsonString(const std::string& is, size_t& pos, std::string& ret)
{
    ...
    catch (const BadQuoteString& e)
    {
        SHARELIB_JSON_THROW(JsonParserError, std::string("BadQuoteString:") + e.what());
    }
    SkipExact(is, pos, "\"");
}
...
{
    typeof(ret.begin()) insPos = ret.begin();
    ...
    for (; pos < is.size(); SkipSpace(is, pos))
    {
        ...
        default:
            ...
            //std::pair<typeof(ret.begin()), bool> it = 
