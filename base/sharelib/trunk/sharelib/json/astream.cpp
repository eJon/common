#include <inttypes.h>
#include <sharelib/json/astream.h>
...
uint32_t a_Istream::ReadVarint(uint32_t& udata)
{
    ...
    {
        byteRoom = get();
        if (byteRoom < 0) SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint32");
        udata |= (byteRoom & 0x7f) << (7 * bytes++); 
        if (byteRoom & 0x80) return bytes;
    }
    SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint32"); 
}
...
uint32_t a_Istream::ReadVarint(uint64_t& udata)
{
    ...
    {
        byteRoom = get();
        if (byteRoom < 0) SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint64");
        udata |= (byteRoom & 0x7f) << (7 * bytes++); 
        if (byteRoom & 0x80) return bytes;
    }
    SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint64"); 
}
////////////////////////////////////////////////////////////////////////////
//////////////////////////// for a_IstreamBuffer ///////////////////////////
////////////////////////////////////////////////////////////////////////////
...
uint32_t a_IstreamBuffer::ReadVarint(uint32_t& udata)
{
    GET_VARINT32(mCurPos, mSize, (mData + mCurPos++))
        SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint32 In a_IstreamBuffer.");
}
...
uint32_t a_IstreamBuffer::ReadVarint(uint64_t& udata)
{
    GET_VARINT64(mCurPos, mSize, (mData + mCurPos++))
        SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint64 In a_IstreamBuffer.");
}
...
int ByteArrayIstream::ReadVarint(uint32_t& udata)
{
    ptrdiff_t remainInBuff = mStop - mCurr;
    /// there are plenty of data in buffer, so we can get from buffer straightforward
    if (remainInBuff >= 5)
    {
        DECODE_VARINT32_DIRECTLY(mCurr)
            }
    else
    {
        GET_VARINT32(mCurr, mStop, mCurr++)
            }
    SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint32 In ByteArrayIstream.");
}
...
int ByteArrayIstream::ReadVarint(uint64_t& udata)
{
    ptrdiff_t remainInBuff = mStop - mCurr;
    /// there are plenty of data in buffer, so we can get from buffer straightforward
    if (remainInBuff >= 10)
    {
        DECODE_VARINT64_DIRECTLY(mCurr)
            }
    else
    {
        GET_VARINT64(mCurr, mStop, mCurr++)
            }
    SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint64 In ByteArrayIstream.");
}
...
uint32_t a_IstreamLengthed::ReadVarint(uint32_t& udata)
{
    ...
    SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint32 In a_IstreamLenghted.");
}
...
uint32_t a_IstreamLengthed::ReadVarint(uint64_t& udata)
{
    ...
    SHARELIB_JSON_THROW(StreamCorruptedException, "ReadVarint64 In a_IstreamLenghted.");
}
