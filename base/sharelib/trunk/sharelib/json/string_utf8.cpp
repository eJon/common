...
Utf8StringIterator::Utf8StringIterator(std::string string) : mString(string), currPos(0)
{        
}

std::string Utf8StringIterator::NextChar()
{
    if (currPos < mString.size())
    {
        if((mString[currPos] & 0x80))
        {
            ...
            {
                SHARELIB_JSON_THROW(ParameterInvalidException, "string with illegal uft8 character");
            }
        }
        else
        {
...
std::string Utf8Substr(const std::string& str, size_t pos, size_t length)
{
...
{
#define trueThenThrow(test, str) if (test) SHARELIB_JSON_THROW(ParameterInvalidException, str);
