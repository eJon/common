#ifndef SHARELIB_JSON_ASTREAM_H
#define SHARELIB_JSON_ASTREAM_H
...
public:
    ...
    {
        if (get() < 0)
            SHARELIB_JSON_THROW(ExceptionBase, "failed to get in GetThenPeek");
        return peek();
    }
...
class a_OstreamMd5 : public a_OstreamWrapper
{
    SHARELIB_NS::Md5Stream md5;
public:
    a_OstreamMd5(a_Ostream &base) : a_OstreamWrapper(base)
    {
    }
    ...
    {
        mBase.put(data);
        md5.Put(reinterpret_cast<const uint8_t*>(&data), sizeof(data));
        return *this;
    }
    ...
    {
        mBase.write(d, len);
        md5.Put(reinterpret_cast<const uint8_t*>(d), len);
    }
        
    void GetMd5(uint8_t buff[16])
    {
        md5.Get(buff);
    }
};
...
class a_IstreamMd5 : public a_IstreamWrapper
{
    Md5Stream md5;
public:
    a_IstreamMd5(a_Istream & base) : a_IstreamWrapper(base)
    {
    }
    ...
    {
        int byte = mBase.get();
        md5.Put(reinterpret_cast<const uint8_t*>(&byte), sizeof(char));
        return byte;
    }

    virtual int32_t read(char* buf, int32_t len)
    {
        int32_t readBytes = mBase.read(buf, len);
        md5.Put(reinterpret_cast<const uint8_t*>(buf), readBytes);
        return readBytes;
    }

    void PutForMd5(const char* buf, int32_t len)
    {
        md5.Put(reinterpret_cast<const uint8_t*>(buf), len);
    }
    ...
    void FetchDataFromBuff(uint32_t bufferSize)
    {
        ...
        if (p != 0)
        {
            ...
            else
            {
                ...
                */
                SHARELIB_JSON_THROW(ExceptionBase, "Bad data requirement since parent has no more data to supply");
            }
        ...
        else // fill buffer
        {
            ...
            {
                SHARELIB_JSON_THROW(ExceptionBase, "no more to read in LengthedStream");
            }
...
public:
    ...
    virtual int GetThenPeek()
    {
        ...
        {
            SHARELIB_JSON_THROW(ExceptionBase, "Bad Use of GetThenPeek().");
        }   
    }
...
{
public:
    ByteArrayIstream(const std::string& str)
    {
        mData = str.data();
        mEnd = mData + str.size();
        mCurr = mData;
        mStop = mEnd;
        mLimit.push_back(mEnd);
    }
    /**
     * @param length: set length limit
     */
    void SetLengthLimit(uint32_t length)
    {
        const char *limit = mCurr + length;
        if (limit > mStop)
        {
            SHARELIB_JSON_THROW(ParameterInvalidException, 
                "the limited length is too large.");
        }
        mLimit.push_back(limit);
        mStop = limit;
    }
    /**
     * release the latest length limit
     */
    void ReleaseLengthLimit()
    {
        if (mLimit.size() > 1)
        {
            if (mCurr != mStop)
            {
                SHARELIB_JSON_THROW(InvalidOperation, 
                    "you haven't consumed all the limited bytes.");
            }
            mLimit.pop_back();
            mStop = mLimit.back();
        }
        ...
        {
            SHARELIB_JSON_THROW(InvalidOperation, "there isn't any length limit to be released.");
        }
    }
    /**
     * Call this method before deconstructor to validate if all length limit have been released
     * and all data has been consumed.
     */
    void CheckDone()
    {
        if (mLimit.size() != 1 || mCurr != mEnd)
        {
            SHARELIB_JSON_THROW(RuntimeError, "Error happened from length limit in BytaArrayIstream.");
        }
    }
    ...
    /**
     * Get ont byte from byte array, if no more to supply, an exception will be throwed
     */
    int Get()
    {
        if (mCurr == mStop)
        {   
            SHARELIB_JSON_THROW(UnexpectedEndOfStreamException, 
                "no more data to supply in ByteArrayIstream.");
        }
        return *mCurr++;
    }
    int Peek()
    ...
    /**
     * Below method will try to read len bytes from input stream at most.
     * When data in input stream is not enough to supply len bytes, it will
     * read all data left and then return the data size it read actually.
     */
    int TryRead(char* buff, int len)
    {
        if (len < 0)
        {
            SHARELIB_JSON_THROW(ParameterInvalidException, "Read length can't be negative.");
        }
        std::ptrdiff_t left = mStop - mCurr;
        int canRead = left < len ? left : len;
        memcpy(buff, mCurr, canRead);
        mCurr += canRead;
        return canRead;
    }
...
#define FULLY_CHECK                                                       \
    ...
    {                                                                     \
        mCurr = mStop;                                                    \
        SHARELIB_JSON_THROW(UnexpectedEndOfStreamException,                      \
            "not enough data to supply in ByteArrayIstream.");            \
    }
    /** 
     * Read exactly len bytes from byte array, if there is not enough data to supply,
     * we will consume all valid data and then throw an exception.
     */
    void ReadFully(char* buff, int len)
    {
        FULLY_CHECK
        memcpy(buff, mCurr, len);
        mCurr += len;
    }
    /**
     * ReadString is ReadFully with parameter of std::string&
     */
    void ReadString(std::string& str, int len)
    {
        FULLY_CHECK
        str.assign(mCurr, len);
        mCurr += len;
    }
    /**
     * Skip the next length bytes
     */
    ...
    /**
     * Fully read 4 bytes from underlying byte array and assign to a uint32_t data.
     */
    int ReadFixed(uint32_t& udata)
    {
        if (mStop - mCurr < 4)
        {
            // consume all data, so user can release length limited
            mCurr = mStop;
            SHARELIB_JSON_THROW(UnexpectedEndOfStreamException, 
                "no more data to supply in ByteArrayIstream.");
        }
        udata = *reinterpret_cast<const uint32_t*>(mCurr);
        mCurr += 4;
        return 4;
    }
    /// @overload for int32_t 
    int ReadFixed(int32_t& data)
    {
        return ReadFixed(reinterpret_cast<uint32_t&>(data));
    }
    /// @overload for uint64_t
    int ReadFixed(uint64_t& udata)
    {
        if (mStop - mCurr < 8)
        {
            // consume all data, so user can release length limited
        ...
        udata = *reinterpret_cast<const uint64_t*>(mCurr);
        mCurr += 8;
        return 8;
    }
    /// @overload for int64_t
    int ReadFixed(int64_t& data)
    {
        return ReadFixed(reinterpret_cast<uint64_t&>(data));
    }

    /**
     * Parse varint byte by byte until we decoded on whole uint32_t data 
     * or an exception will be throwed
     */
    int ReadVarint(uint32_t& udata);
    /// @overload fro parameter uint64_t
    int ReadVarint(uint64_t& udata);
    /**
     * Below method will calculate md5 for data between 
     * offset "begin" and current position
     */
    void GetMd5(int begin, uint8_t md5[16])
    {
        const char * start = mData + begin;
        std::ptrdiff_t size = mCurr - start;
        if (size < 0)
        {
            SHARELIB_JSON_THROW(InvalidOperation, 
                "no data input when calculate md5 in ByteArrayIstream.");
        }
        DoMd5(reinterpret_cast<const uint8_t*>(start), size, md5);
    }
    /**
     * Below method will return the offset of mCurr.
     */
    int Tell()
    {
        return mCurr == mEnd ? -1 : mCurr - mData;
    }
    /**
     * @NOTE: 
     * By design, Seek(pos) will not consider length limited to simplify the implementation.
     * And it is enough to match cangjie's requirement for seek back at the beginning in FromStream.
     * 
     * Below method seek position from beginning with offset of pos.
     */
    void Seek(int pos)
    {
        std::ptrdiff_t size = mEnd - mData;
        if (pos < 0 || pos >= size)
        {
            SHARELIB_JSON_THROW(ParameterInvalidException, "seek position is invalid.");
        }
        mCurr = mData + pos;
    }
private:
    /**
     * @param mData: the head pointer of the underlying byte array.
     * @param mCurr: a pointer to the first byte can read.
     * @param mEnd:  a pointer to one past the end of the underlying byte array.
     * @param mStop: a pointer to one past the end of length limited input stream,
     *               when user never set the limited length, mStop == mEnd.
     */
    const char* mData, * mEnd, * mCurr, * mStop;
    std::vector<const char *> mLimit;
};
